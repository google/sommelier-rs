...
1. **Interception:** Guest applications allocate POSIX Shared Memory (memfds) and send them via `wl_shm.create_pool` to the proxy. The proxy duplicates the FD and mmaps the shared memory but defers hardware allocation.
2. **Hardware Allocation:** When the guest subsequently issues a `wl_shm_pool.create_buffer` call, the proxy intercepts this and allocates a hardware-backed `dma-buf` buffer via `virtio-gpu` (with `MAPPABLE` and `SHAREABLE` flags, or via the `gbm` allocator as a fallback) for that specific buffer.
3. **Mappable FD Transmission:** Because the `virtio-gpu` dma-buf is mappable, the proxy treats it as a standard SHM pool. It duplicates the `dma-buf` FD and sends it to the Host Compositor by creating a *new* temporary `wl_shm.create_pool` specifically for this buffer, followed immediately by `wl_shm_pool.create_buffer` to create the host-side buffer, and then immediately destroys the temporary host pool. The Host Compositor uses `wl_shm` entirely unaware that it is receiving a hardware-backed `dma-buf`.
...
* **Target Environment:** Inside a Linux Virtual Machines Guest.
* **Key Services Used:** Wayland, DRM/KMS, virtio-gpu.
* **CI/CD Pipeline:** Not there yet

## 7. Security Considerations

* **Protocol Filtering (Safelist):** The proxy intercepts `wl_registry.global` events from the host. It only forwards known, supported protocols to the guest. This prevents guest applications from binding to unparseable extensions which could desynchronize the proxy.
...
* **CI/CD:** Add CI/CD to ensure its correctness and produce release build binaries for easy onboarding.

## 10. Project Identification

* **Project Name:** sommelier-rs
* **Repository URL:** <https://github.com/google/sommelier-rs>
